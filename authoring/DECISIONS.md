...
The initial register was resolved with the project owner on 2026-07-06. Later amendments are dated in the affected rows. Resolved decisions are policy for guideline pages; add or amend a row here before changing policy.
...
| CFG-4 | Type-check gate (Bun and Vite do not type check) | (a) `tsc --noEmit` pre-commit and in CI; (b) CI only; (c) editor diagnostics only | (a) | (a); in a workspace, the root `typecheck` script orchestrates checks against each package tsconfig, which extends the root baseline (amended 2026-07-10) |
...
| CFG-14 | `useLiteralKeys` under `noPropertyAccessFromIndexSignature` | (a) disable `useLiteralKeys`; (b) keep it and suppress per site | (a) - CFG-1 mandates the tsconfig flag that forces the bracket access this rule flags | (a) (added 2026-08-28) |
| CFG-15 | Lint file-exclusion source of truth | (a) `vcs.useIgnoreFile` so `.gitignore` governs; (b) hand-maintained `files.includes` lists | (a) - one ignore list instead of two | (a); keep `files.includes` entries only for committed generated files (added 2026-08-28) |
| CFG-16 | Rule relaxation mechanism | (a) scoped `overrides` blocks for narrow file sets; (b) weaken root rules | (a) - keeps the root config strict | (a) (added 2026-08-28) |
...
| REACT-2 | React 19 idioms and libraries | ref-as-prop vs `forwardRef`; state library standard; data-fetching standard; styling approach | ref-as-prop (React 19); remaining choices need owner input | ref-as-prop; Tailwind CSS is the styling standard; SWR is the data-fetching standard when a library is warranted, with named query/mutation hooks in a central module (fabro precedent); Zustand is the standard for state shared across deeply nested components once lifting state is insufficient (amended 2026-07-07) |
| REACT-3 | `useEffect` policy | (a) ban direct `useEffect`/`useLayoutEffect`: derive during render, act in event handlers, fetch with SWR, reset with `key`, single `useMountEffect` wrapper for mount-only external sync; (b) allowed with review | (a) - Factory AI precedent for agent-maintained code; mechanizes the React team's "You Might Not Need an Effect" | (a), enforced with Biome `noRestrictedImports` in frontend packages; rare non-mount effects wrap `useEffect` in a purpose-named hook with a documented suppression (CFG-8) (added 2026-07-07) |
